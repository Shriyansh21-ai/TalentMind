...
def build_provenance(
    ...
    * Recommendation — the forward-looking interview focus.
    ...
    for label in available_sources(evidence):
        entries.append(
            ProvenanceEntry(
                kind="Evidence", statement=f"Consumed structured output from {label}.", source=label
            )
        )
    ...
    if narrative.personalization_note:
        entries.append(
            ProvenanceEntry(
                kind="Inference",
                statement=narrative.personalization_note,
    ...
    if narrative.recommended_focus:
        entries.append(
            ProvenanceEntry(
                kind="Recommendation",
                statement=narrative.recommended_focus,
                source="Interview Studio",
            )
        )
