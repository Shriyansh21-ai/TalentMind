...
def compose_compliance_narrative(evidence: Dict[str, Any]) -> Dict[str, Any]:
    ...
    summary = (
        ...
        + (
            ...
            if review.get("reviewers") else "No elevated Legal/Compliance review indicated. "
        )
    ...
    assumptions.append("Compliance findings support human review; they are not a legal determination.")

    if wf_status == "Compliant" and risk_level == "Low":
        confidence_note = "Confidence is solid: the evidenced workflow is complete and no exceptions surfaced."
    ...
    return {
        ...
        "approval_note": (
            ...
            + (f"Outstanding: {', '.join(outstanding)}." if outstanding else "No outstanding approvals.")
        ),
        "policy_note": (
            "; ".join(f"{c.get('policy_name')}: {c.get('status')}" for c in policy_checks)
            if policy_checks else "No policies evaluated."
        ),
        "documentation_note": (
            f"Missing documents: {', '.join(missing_docs)}." if missing_docs
        ...
        "risk_note": (
            f"Governance risk is {risk_level}. Drivers: " + "; ".join(risk.get("drivers", [])[:3]) + "."
        ),
        "required_actions": required_actions or ["Proceed through standard approvals."],
        "key_findings": key_findings,
        "assumptions": assumptions,
        "human_review_recommendations": risk.get("human_review_recommendations", [])[:5] or ["Standard approvals only."],
