...
_RESP_ORDER = {"junior": 1, "mid": 2, "senior": 3, "staff": 4, "principal": 5, "lead": 4, "manager": 4}
...
def assess_inversion(context: Dict[str, Any], provider: Any) -> InversionAssessment:
    ...
    if not cases:
        level = "Low"
        rationale = "No existing peer with equivalent-or-greater responsibility earns below the offer."
    elif len(cases) >= 3:
        level = "High"
        rationale = f"{len(cases)} peers with equivalent-or-greater responsibility earn below the offer."
    else:
        level = "Medium"
        rationale = f"{len(cases)} peer(s) with equivalent-or-greater responsibility earn below the offer."
    ...
    return InversionAssessment(
        ...
        business_impact=(
            ...
            "peer adjustments." if cases else "No inversion signal detected."
        ),
        recommended_review=(
            "Route to HR compensation to review the affected peers before extending the offer."
            if cases else "No review required on inversion grounds."
        ),
    )
