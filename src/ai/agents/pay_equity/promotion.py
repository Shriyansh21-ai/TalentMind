...
def assess_promotion_equity(context: dict[str, Any], provider: Any) -> PromotionEquityAssessment:
    ...
    if data_available:
        ...
        if hi:
            within = lo <= target <= hi
            level_alignment = f"Offer target sits {'within' if within else 'outside'} the {context.get('level', 'role')} band."
            consistency = "Consistent" if within else "Review"
            if not within:
                recommendations.append(
                    "Reconcile the level assignment with the published band before promotion parity is set."
                )
            confidence = 70.0
        else:
        ...
        level_alignment = (
            "Internal level bands unavailable; level alignment not evaluable without HRIS data."
        )
        consistency = "Not Evaluable"
        recommendations.append(
            "Connect an HRIS source to evaluate internal promotion/level consistency."
        )

    if growth >= 70:
        recommendations.append(
            "Strong growth trajectory — plan an accelerated review cadence to keep pay aligned."
        )
