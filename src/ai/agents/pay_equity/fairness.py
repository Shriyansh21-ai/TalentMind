...
def build_fairness_assessment(
    ...
    if not data_available:
        ...
        assumptions.append(
            "No internal payroll/HRIS data available; internal-equity checks are deferred."
        )
        ...
        if inversion.risk_level in ("Medium", "High"):
            concerns.append(
                f"{inversion.risk_level} pay-inversion risk vs. equivalent-responsibility peers."
            )
            review_recs.append(inversion.recommended_review)

    if policy_alignment.alignment in ("Partial", "Violation"):
        concerns.append(
            f"Policy alignment is {policy_alignment.alignment} under '{policy_alignment.policy_name}'."
        )
    ...
    return FairnessAssessment(
        ...
        or ["No material fairness concerns surfaced from the available data."],
        human_review_recommendations=list(dict.fromkeys(r for r in review_recs if r))
        or ["No human review required beyond standard approvals."],
        governance_notes=governance_notes,
        evidence=list(dict.fromkeys(evidence)),
        assumptions=assumptions
        or ["Assessment rests on the connected data and the existing intelligence signals."],
    )
