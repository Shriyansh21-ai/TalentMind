...
def build_team_metrics(cohort: List[Dict[str, Any]], provider: Any, data_available: bool) -> List[TeamMetric]:
    ...
    if data_available and provider is not None and hasattr(provider, "get_team_metrics"):
        for row in provider.get_team_metrics() or []:
            metrics.append(
                TeamMetric(
                    dimension=row.get("dimension", "Team"), group=row.get("group", "?"),
                    count=int(row.get("count", 0)), hiring_health=row.get("hiring_health", "n/a"),
                    register="Observed", detail=row.get("detail", "From the connected analytics source."),
                )
            )
    else:
        for dim in _PROVIDER_DIMENSIONS:
            metrics.append(
                TeamMetric(dimension=dim, group="(all)", count=0, hiring_health="Unavailable",
                           register="Unavailable", detail="Requires a connected org-structure / analytics source.")
            )
    return metrics
