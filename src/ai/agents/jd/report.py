...
_SENIORITY = ["intern", "junior", "associate", "mid", "engineer", "senior", "staff", "lead", "principal", "manager", "director", "vp", "head", "chief", "architect"]
...
def build_role(ev: Dict[str, Any]) -> Dict[str, Any]:
    ...
    blob = (doc.get("title", "") + " " + " ".join(doc.get("responsibilities", []) + doc.get("requirements", []))).lower()
    ...
    architecture = _has(blob, "architect", "system design", "design systems", "scalab", "distributed")
    customer = _has(blob, "customer", "client", "stakeholder", "user-facing")
    management = _has(blob, "manage a team", "direct reports", "people management", "hire", "line manage")
    cross = _has(blob, "cross-functional", "collaborate", "partner with", "work with product", "stakeholder")
...
def build_technical(ev: Dict[str, Any]) -> Dict[str, Any]:
    ...
    maturity = "Modern" if modern and not dated else ("Mixed" if modern and dated else ("Legacy-leaning" if dated else "Unspecified"))
    diversity = "High" if m.get("tech_count", 0) >= 12 else ("Moderate" if m.get("tech_count", 0) >= 6 else "Focused")
    observations = []
    if modern:
    ...
    if m.get("tech_count", 0) >= 15:
        observations.append("Very broad tech list — inference: role may be under-scoped or a 'unicorn' ask.")
    return {
...
_INTENT_CUES = {
    "Growth hiring": (("grow", "growing", "expand", "scaling the team", "new team", "build a team", "scale up"), "Language about team/company growth."),
    "Replacement hiring": (("backfill", "replace", "vacancy", "departing"), "Explicit backfill/replacement language."),
    "Innovation hiring": (("greenfield", "0 to 1", "zero to one", "new product", "innovat", "research", "prototype", "cutting edge"), "New-product / greenfield / innovation language."),
    "Platform modernization": (("modernize", "migrate", "legacy", "re-platform", "refactor", "rearchitect", "modernization"), "Modernization / migration language."),
    "Digital transformation": (("transformation", "digitize", "digital transformation", "cloud migration"), "Transformation language."),
    "Cost optimization": (("cost optimization", "efficiency", "reduce cost", "optimize spend", "consolidat"), "Cost/efficiency language."),
}
...
def build_hiring_intent(ev: Dict[str, Any]) -> Dict[str, Any]:
    ...
    doc = ev.get("document", {})
    blob = ev.get("text_blob") or " ".join(
        [doc.get("title", ""), doc.get("company_info", "")]
        + doc.get("responsibilities", []) + doc.get("requirements", [])
    ).lower()
    ...
    for intent, (cues, rationale) in _INTENT_CUES.items():
        ...
        if hits:
            ...
            signals.append(
                {"intent": intent, "rationale": f"{rationale} (matched: {', '.join(hits)})", "confidence": round(confidence, 1)}
            )

    if not signals:
        signals.append(
            {"intent": "Capacity hiring", "rationale": "No explicit intent cues; likely adding capacity to an existing function.", "confidence": 35.0}
        )
...
_ORG_CUES = {
    "Startup": ("startup", "early-stage", "seed", "series a", "fast-paced", "wear many hats", "founding"),
    "Scale-up": ("series b", "series c", "hyper-growth", "scale-up", "scaleup", "scaling rapidly"),
    "Enterprise": ("enterprise", "fortune", "global", "large-scale", "multinational", "established"),
...
def build_organization(ev: Dict[str, Any]) -> Dict[str, Any]:
    ...
    signals = [f"{label}: {', '.join(c for c in cues if c in blob)}" for label, cues in _ORG_CUES.items() if scored[label]]

    tech_maturity = "High" if m.get("modern_tech") and (m.get("devops") or m.get("cloud")) else ("Moderate" if m.get("all_technologies") else "Unclear")
    eng_maturity = "High" if (m.get("devops") and m.get("architecture")) else ("Moderate" if m.get("devops") or m.get("architecture") else "Unclear")
    confidence = min(80.0, 40.0 + (max(scored.values()) if scored else 0) * 12.0)

    observations = ["Company type is inferred from JD language; verify against actual company profile."]
    return {
        "company_type": best,
...
_MANDATORY_CUES = ("must", "required", "strong", "solid", "proven", "expert", "essential", "minimum", "years")
_PREFERRED_CUES = ("preferred", "nice to have", "nice-to-have", "bonus", "plus", "good to have", "desirable", "ideally", "familiarity")
...
def generate_improvements(ev: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not doc.get("requirements"):
        improvements.append(_imp("Add explicit requirements", "Requirements", "high", "Recruiters cannot screen without clear qualifications.", "List 4-6 must-have qualifications."))
    if not doc.get("responsibilities"):
        improvements.append(_imp("Add a responsibilities section", "Role Clarity", "high", "Candidates can't self-assess fit without day-to-day scope.", "Add 'What you'll do' with 4-6 bullets."))
    if not doc.get("preferred"):
        improvements.append(_imp("Split mandatory vs preferred skills", "Requirement Quality", "high", "A flat list inflates the bar and shrinks the funnel.", "Separate 'Must have' from 'Nice to have'."))
    if not doc.get("compensation"):
        improvements.append(_imp("Add a compensation range", "Hiring Readiness", "medium", "Comp transparency improves applies and reduces late-stage drop-off.", "Add a band, even if broad."))
    if m.get("tech_count", 0) >= 15:
        improvements.append(_imp("Reduce technology overload", "Technical Clarity", "medium", "A long tech list reads as an unfillable 'unicorn' role.", "Keep 5-8 core technologies; move the rest to 'nice to have'."))
    if dims.get("role_clarity", 100) < 55:
        improvements.append(_imp("Clarify ownership & success metrics", "Role Clarity", "medium", "Ownership and outcomes are unclear.", "Add 'success in 6 months looks like…'."))
    if m.get("vague_terms"):
        improvements.append(_imp("Reduce ambiguity / filler", "Quality", "low", "Vague filler weakens clarity.", "Cut: " + ", ".join(m["vague_terms"]) + "."))
    if m.get("bias_terms"):
        improvements.append(_imp("Improve inclusiveness", "Quality", "high", "Potentially biased/exclusionary language detected.", "Remove: " + ", ".join(m["bias_terms"]) + "."))
    if not (m.get("architecture")) and (doc.get("years_experience") or 0) >= 6:
        improvements.append(_imp("Clarify architecture expectations", "Role Clarity", "low", "Senior role but architecture scope is unstated.", "State the system-design scope expected."))
...
def _imp(title: str, area: str, priority: str, rationale: str, example: str) -> Dict[str, Any]:
    ...
    return {"title": title, "area": area, "priority": priority, "impact": impact, "rationale": rationale, "example": example}
