...
def generate_summary(
    ...
    summary.append(
        f"Matches {skill_gap['match_percent']}% of JD skills."
    )

    if len(skill_gap["missing"]) == 0:

        summary.append(
            "All critical JD skills are present."
        )

    elif len(skill_gap["missing"]) <= 2:

        summary.append(
            "Only minor skill gaps detected."
        )

    else:

        summary.append(
            f"Missing {len(skill_gap['missing'])} important JD skills."
        )
    ...
    if explanation["experience_score"] >= 15:
        ...
        summary.append(
            "Experience level aligns well with role expectations."
        )
