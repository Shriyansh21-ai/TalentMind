...
def generate_recruiter_reason(candidate):
    ...
    if candidate.profile.years_of_experience >= 5:
        reasons.append(f"{candidate.profile.years_of_experience} years of relevant experience")

    title = candidate.profile.current_title.lower()

    if any(
        keyword in title for keyword in ["ai", "ml", "machine learning", "nlp", "recommendation"]
    ):
        reasons.append(
    ...
    if calculate_career_score(candidate) >= 20:
        reasons.append("Career history includes retrieval, ranking, recommendation or LLM projects")

    if calculate_jd_match_score(candidate) >= 30:
        reasons.append("Strong match with job requirements")

    if calculate_behavior_score(candidate) >= 25:
        reasons.append("Positive recruiter engagement signals")
    ...
    if calculate_red_flag_penalty(candidate) < 0:
        reasons.append("Some hiring risks detected")
