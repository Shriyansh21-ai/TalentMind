...
def _engine() -> AuthorizationEngine:
    ...
    engine.hierarchy.define_role("t1", "o1", "hr_director", inherits=["recruiter"], permissions=["compensation:approve"])
    return engine
...
def test_rbac_inheritance():
    ...
    req2 = AuthorizationRequest(tenant_id="t1", roles=["hr_director"], permission="compensation:approve")
...
def test_abac_allow_policy_grants_without_role():
    engine = _engine()
    engine.add_policy(
        "t1", "o1", "owner-can-read", effect=PolicyEffect.ALLOW,
        resource="report", action="read",
        conditions=[AttributeCondition(attribute="resource.owner", operator=AttributeOperator.EQ, value="alice")],
    )
    req = AuthorizationRequest(
        tenant_id="t1", subject="alice", roles=[], permission="report:read",
...
def test_abac_deny_overrides_rbac():
    engine = _engine()
    engine.add_policy(
        "t1", "o1", "no-offhours", effect=PolicyEffect.DENY,
        resource="candidate", action="update",
        conditions=[AttributeCondition(attribute="environment.after_hours", operator=AttributeOperator.EQ, value=True)],
        priority=100,
    )
    req = AuthorizationRequest(
        tenant_id="t1", roles=["recruiter"], permission="candidate:update",
